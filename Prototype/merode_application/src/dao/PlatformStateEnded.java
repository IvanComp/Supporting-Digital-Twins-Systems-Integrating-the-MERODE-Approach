 ...
 */
public class PlatformStateEnded extends PlatformState {

    public static PlatformStateEnded getObject (org.hibernate.Session sess) throws org.hibernate.HibernateException {
	    PlatformStateEnded state = null;
	    // Search in database
	    java.util.Collection states = findStateByName (sess, "ended");
	    if ( states != null && !states.isEmpty() )
	        state = (PlatformStateEnded)states.iterator().next();
	    if ( state == null ) {
	        state = new PlatformStateEnded();
	        // Save in database
	        sess.save (state);
	    }
	    return state;
