 ...
 */
public class ProcedureStateEnded extends ProcedureState {

    public static ProcedureStateEnded getObject (org.hibernate.Session sess) throws org.hibernate.HibernateException {
	    ProcedureStateEnded state = null;
	    // Search in database
	    java.util.Collection states = findStateByName (sess, "ended");
	    if ( states != null && !states.isEmpty() )
	        state = (ProcedureStateEnded)states.iterator().next();
	    if ( state == null ) {
	        state = new ProcedureStateEnded();
	        // Save in database
	        sess.save (state);
	    }
	    return state;
