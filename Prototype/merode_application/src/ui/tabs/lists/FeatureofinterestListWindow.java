...
import javax.swing.*;
import ui.tabs.operations.create.FeatureofinterestmecrfeatureofinterestWindow;
...
public class FeatureofinterestListWindow extends ObjectListWindow {
    public FeatureofinterestListWindow (MerodeMainEventHandlerGui main_frame, 
    		MerodeMainEventHandler main_handler) {
        super (main_frame, main_handler, "Featureofinterest");
    }

    public FeatureofinterestListWindow (MerodeMainEventHandlerGui main_frame, 
    		MerodeMainEventHandler main_handler, 
    		String id) {
        super (main_frame, main_handler, "Featureofinterest", id);
    }
    
    public FeatureofinterestListWindow(MerodeMainEventHandlerGui main_frame,
			MerodeMainEventHandler main_handler, Collection<?> col) {
        super (main_frame, main_handler, "Featureofinterest", col);
	}

	protected void completeMethodPane() {
        JButton b_mecrfeatureofinterest = new JButton("mecrfeatureofinterest");
        b_mecrfeatureofinterest.addActionListener( new ActionListener() {
            public void actionPerformed (ActionEvent event) {
                execute_mecrfeatureofinterest();
            }
        });
        c_methods_p.add (b_mecrfeatureofinterest);  


        JButton b_meendfeatureofinterest = new JButton("meendfeatureofinterest");
        b_meendfeatureofinterest.addActionListener( new ActionListener() {
            public void actionPerformed (ActionEvent event) {
                execute_meendfeatureofinterest();
            }
        });
        e_methods_p.add (b_meendfeatureofinterest);  
    ...
    private void execute_mecrfeatureofinterest() {
        setVisible(false);

        FeatureofinterestmecrfeatureofinterestWindow w_mecrfeatureofinterest = new FeatureofinterestmecrfeatureofinterestWindow (main_frame, main_handler, this);
        w_mecrfeatureofinterest.pack();
        //w_mecrfeatureofinterest.setSize(300,w_mecrfeatureofinterest.getHeight());
        w_mecrfeatureofinterest.setVisible(true);
    }
    

    private void execute_meendfeatureofinterest() {
        try {
            if (table.getSelectedRow()>=0) {
            		            Object object = table.objects.get(table.getSelectedRow());
	            if (!object.getClass().equals(dao.FeatureofinterestImpl.class)){
		            ...
		            if (JOptionPane.showConfirmDialog(null,"Are you sure you want to end this object?","Question",JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
		                main_handler.meendfeatureofinterest(((dao.Featureofinterest) object).getId());
    ...
    protected void constructTable() {
        table = new Featureofinterest_Table(main_handler);
        addTableListeners();
    }

    protected void constructTable(String id) {
        table = new Featureofinterest_Table(main_handler, id);
        addTableListeners();
    }

	protected void constructTable(Collection<?> col) {
		table = new Featureofinterest_Table(main_handler, col);
		addTableListeners();
	...
	private void addTableListeners() {
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				dao.Featureofinterest object = (dao.Featureofinterest)table.objects.get(row);
