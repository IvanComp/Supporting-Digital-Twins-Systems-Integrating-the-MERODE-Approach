...
import javax.swing.*;
import ui.tabs.operations.create.PropertymecrpropertyWindow;
...
public class PropertyListWindow extends ObjectListWindow {
    public PropertyListWindow (MerodeMainEventHandlerGui main_frame, 
    		MerodeMainEventHandler main_handler) {
        super (main_frame, main_handler, "Property");
    }

    public PropertyListWindow (MerodeMainEventHandlerGui main_frame, 
    		MerodeMainEventHandler main_handler, 
    		String id) {
        super (main_frame, main_handler, "Property", id);
    }
    
    public PropertyListWindow(MerodeMainEventHandlerGui main_frame,
			MerodeMainEventHandler main_handler, Collection<?> col) {
        super (main_frame, main_handler, "Property", col);
	}

	protected void completeMethodPane() {
        JButton b_mecrproperty = new JButton("mecrproperty");
        b_mecrproperty.addActionListener( new ActionListener() {
            public void actionPerformed (ActionEvent event) {
                execute_mecrproperty();
            }
        });
        c_methods_p.add (b_mecrproperty);  


        JButton b_meendproperty = new JButton("meendproperty");
        b_meendproperty.addActionListener( new ActionListener() {
            public void actionPerformed (ActionEvent event) {
                execute_meendproperty();
            }
        });
        e_methods_p.add (b_meendproperty);  
    ...
    private void execute_mecrproperty() {
        setVisible(false);

        PropertymecrpropertyWindow w_mecrproperty = new PropertymecrpropertyWindow (main_frame, main_handler, this);
        w_mecrproperty.pack();
        //w_mecrproperty.setSize(300,w_mecrproperty.getHeight());
        w_mecrproperty.setVisible(true);
    }
    

    private void execute_meendproperty() {
        try {
            if (table.getSelectedRow()>=0) {
            		            Object object = table.objects.get(table.getSelectedRow());
	            if (!object.getClass().equals(dao.PropertyImpl.class)){
		            ...
		            if (JOptionPane.showConfirmDialog(null,"Are you sure you want to end this object?","Question",JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
		                main_handler.meendproperty(((dao.Property) object).getId());
    ...
    protected void constructTable() {
        table = new Property_Table(main_handler);
        addTableListeners();
    }

    protected void constructTable(String id) {
        table = new Property_Table(main_handler, id);
        addTableListeners();
    }

	protected void constructTable(Collection<?> col) {
		table = new Property_Table(main_handler, col);
		addTableListeners();
	...
	private void addTableListeners() {
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				dao.Property object = (dao.Property)table.objects.get(row);
