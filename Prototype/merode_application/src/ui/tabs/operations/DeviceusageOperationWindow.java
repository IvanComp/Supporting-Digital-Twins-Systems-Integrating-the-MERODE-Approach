 ...
 */
public abstract class DeviceusageOperationWindow extends JFrame {
    ...
    public DeviceusageOperationWindow (MerodeMainEventHandlerGui main_f, 
    				MerodeMainEventHandler main_h, 
    					ObjectListWindow list_f, 
    						String b_name) {
        super("Deviceusage: " + b_name);
        main_frame = main_f;
        main_handler = main_h;
        list_frame = list_f;
        button_name = b_name;
    ...
    protected void buildFrame(Container p) {
        ...
        constructplatformdeploymentTable(p);
        constructregistereddeviceTable(p);
        constructprocedureTable(p);
        constructpropertyTable(p);
		constructButtonPane(p);
    }

    protected abstract void constructplatformdeploymentTable(Container p);
    protected abstract void constructregistereddeviceTable(Container p);
    protected abstract void constructprocedureTable(Container p);
    protected abstract void constructpropertyTable(Container p);
