...
public class Deviceresult_Table extends ObjectTable {
    private MerodeMainEventHandler main_handler;
    private MerodeMainEventHandlerGui parent_win;

    public Deviceresult_Table(MerodeMainEventHandler main_h) {
        this (null, main_h);
    }

    public Deviceresult_Table(MerodeMainEventHandlerGui par_win, 
    ...
    public Deviceresult_Table(MerodeMainEventHandler main_h, String object_id) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            try {
            	obj = main_handler.searchDeviceresultById (object_id);
            } catch (Exception ex){
    ...
    public Deviceresult_Table(MerodeMainEventHandler main_h, Collection col) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            Collections.sort(objects, new Comparator<Object>() {
				...
				public int compare(Object arg0, Object arg1) {
					return (((dao.Deviceresult) arg0).getId()
						.compareTo(((dao.Deviceresult) arg1).getId()));
				}
            });
            createColumns();
            attachRenderers();
	...
	protected void getAllObjects() {
        try {
            Collection c = main_handler.getAllDeviceresult();
    ...
    protected void createRows() {
    	...
    	try {
            ...
            for (int i = 0; i < all.size(); i++){
            	keys.put(new Integer(i), ((dao.Deviceresult)all.get(i)).getId());
            }
		...
		if ( all != null ){
			if(all.size() > objects.size() && keys != null){
				for (int i = 0; i < objects.size(); i++) {
					...
					for (Entry<Integer, String> entry : keys.entrySet()) {
						if (entry.getValue().equals(
								((dao.Deviceresult) objects.get(i)).getId())) {
							v.add(entry.getKey());
						}
					}
					getRowData((dao.Deviceresult) objects.get(i), v);
					model.addRow(v);
				}
			} else {
		        ...
		        for (int i = 0; i < objects.size(); i++) {
		            Vector v = new Vector();
		            v.add (new Integer(i)
		            	+ ":" + ((dao.Deviceresult) objects.get(i)).getMandatoryInconsistency());
		            getRowData((dao.Deviceresult)objects.get(i), v);
		            model.addRow(v);
		        }
        	}
    ...
    protected void createColumns() {
        ...
        model.addColumn("<html><b>Time</b></html>");
    ...
    protected void attachRenderers() {
    	...
    	getColumnModel().getColumn(4).setCellEditor(
        		new deviceresultAllCollectionsEditor( parent_win, 
        			main_handler, objects, this)
        );
    	
    }

    private void getRowData (dao.Deviceresult object, Vector vector) {
    
        try {
            	object = main_handler.searchDeviceresultById (object.getId());
            	vector.add (object.getTime()); 
            } catch (Exception ex){
            	System.out.println ("not an instance");
            } 
   ...
   */
    class deviceresultAllCollectionsEditor extends ObjectTableCellEditor {
      ...
      public deviceresultAllCollectionsEditor(
      	  ...
      	  but.addActionListener( new ActionListener() {
      		  public void actionPerformed (ActionEvent event) {
      	    	  ...
      	    	  //but.setEnabled(false);
      	     		dao.Deviceresult object = (dao.Deviceresult)objects.get(table.getSelectedRow());
    	    		String id = object.getId();
    	    		//FIXME - object needs to come fresh
    	    		if (main_handler != null){
    	        		try {
    	    				object = main_handler.searchDeviceresultById (id);
    	    			} catch (Exception e) {
    	    				// TODO Auto-generated catch block
    	    				e.printStackTrace();
    	    			}
    	    	}
    	    	DeviceresultViewWindow theDeviceresultViewWindow =
    	      		new DeviceresultViewWindow (parent_win, main_handler, object);

    	        final JFrame frame = new JFrame();
    	        frame.setTitle("Deviceresult "); /*+ (table.getSelectedRow())*/;
    	        ...
    	        JScrollPane scrollpane = new JScrollPane(theDeviceresultViewWindow);
    	        frame.add(scrollpane);
    	        
    			// this centers the frame on the screen
    			//frame.setLocationRelativeTo(null);
    	        frame.pack();
       			validateAndShowDetailWindow(frame, (dao.Deviceresult)object);
