...
public class Platform_Table extends ObjectTable {
    private MerodeMainEventHandler main_handler;
    private MerodeMainEventHandlerGui parent_win;

    public Platform_Table(MerodeMainEventHandler main_h) {
        this (null, main_h);
    }

    public Platform_Table(MerodeMainEventHandlerGui par_win, 
    ...
    public Platform_Table(MerodeMainEventHandler main_h, String object_id) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            try {
            	obj = main_handler.searchPlatformById (object_id);
            } catch (Exception ex){
    ...
    public Platform_Table(MerodeMainEventHandler main_h, Collection col) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            Collections.sort(objects, new Comparator<Object>() {
				...
				public int compare(Object arg0, Object arg1) {
					return (((dao.Platform) arg0).getId()
						.compareTo(((dao.Platform) arg1).getId()));
				}
            });
            createColumns();
            attachRenderers();
	...
	protected void getAllObjects() {
        try {
            Collection c = main_handler.getAllPlatform();
    ...
    protected void createRows() {
    	...
    	try {
            ...
            for (int i = 0; i < all.size(); i++){
            	keys.put(new Integer(i), ((dao.Platform)all.get(i)).getId());
            }
		...
		if ( all != null ){
			if(all.size() > objects.size() && keys != null){
				for (int i = 0; i < objects.size(); i++) {
					...
					for (Entry<Integer, String> entry : keys.entrySet()) {
						if (entry.getValue().equals(
								((dao.Platform) objects.get(i)).getId())) {
							v.add(entry.getKey());
						}
					}
					getRowData((dao.Platform) objects.get(i), v);
					model.addRow(v);
				}
			} else {
		        ...
		        for (int i = 0; i < objects.size(); i++) {
		            Vector v = new Vector();
		            v.add (new Integer(i)
		            	+ ":" + ((dao.Platform) objects.get(i)).getMandatoryInconsistency());
		            getRowData((dao.Platform)objects.get(i), v);
		            model.addRow(v);
		        }
        	}
    ...
    protected void attachRenderers() {
    	...
    	getColumnModel().getColumn(4).setCellEditor(
        		new platformAllCollectionsEditor( parent_win, 
        			main_handler, objects, this)
        );
    	
    }

    private void getRowData (dao.Platform object, Vector vector) {
    
        try {
            	object = main_handler.searchPlatformById (object.getId());
   ...
   */
    class platformAllCollectionsEditor extends ObjectTableCellEditor {
      ...
      public platformAllCollectionsEditor(
      	  ...
      	  but.addActionListener( new ActionListener() {
      		  public void actionPerformed (ActionEvent event) {
      	    	  ...
      	    	  //but.setEnabled(false);
      	     		dao.Platform object = (dao.Platform)objects.get(table.getSelectedRow());
    	    		String id = object.getId();
    	    		//FIXME - object needs to come fresh
    	    		if (main_handler != null){
    	        		try {
    	    				object = main_handler.searchPlatformById (id);
    	    			} catch (Exception e) {
    	    				// TODO Auto-generated catch block
    	    				e.printStackTrace();
    	    			}
    	    	}
    	    	PlatformViewWindow thePlatformViewWindow =
    	      		new PlatformViewWindow (parent_win, main_handler, object);

    	        final JFrame frame = new JFrame();
    	        frame.setTitle("Platform "); /*+ (table.getSelectedRow())*/;
    	        ...
    	        JScrollPane scrollpane = new JScrollPane(thePlatformViewWindow);
    	        frame.add(scrollpane);
    	        
    			// this centers the frame on the screen
    			//frame.setLocationRelativeTo(null);
    	        frame.pack();
       			validateAndShowDetailWindow(frame, (dao.Platform)object);
