...
public class Procedure_Table extends ObjectTable {
    private MerodeMainEventHandler main_handler;
    private MerodeMainEventHandlerGui parent_win;

    public Procedure_Table(MerodeMainEventHandler main_h) {
        this (null, main_h);
    }

    public Procedure_Table(MerodeMainEventHandlerGui par_win, 
    ...
    public Procedure_Table(MerodeMainEventHandler main_h, String object_id) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            try {
            	obj = main_handler.searchProcedureById (object_id);
            } catch (Exception ex){
    ...
    public Procedure_Table(MerodeMainEventHandler main_h, Collection col) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            Collections.sort(objects, new Comparator<Object>() {
				...
				public int compare(Object arg0, Object arg1) {
					return (((dao.Procedure) arg0).getId()
						.compareTo(((dao.Procedure) arg1).getId()));
				}
            });
            createColumns();
            attachRenderers();
	...
	protected void getAllObjects() {
        try {
            Collection c = main_handler.getAllProcedure();
    ...
    protected void createRows() {
    	...
    	try {
            ...
            for (int i = 0; i < all.size(); i++){
            	keys.put(new Integer(i), ((dao.Procedure)all.get(i)).getId());
            }
		...
		if ( all != null ){
			if(all.size() > objects.size() && keys != null){
				for (int i = 0; i < objects.size(); i++) {
					...
					for (Entry<Integer, String> entry : keys.entrySet()) {
						if (entry.getValue().equals(
								((dao.Procedure) objects.get(i)).getId())) {
							v.add(entry.getKey());
						}
					}
					getRowData((dao.Procedure) objects.get(i), v);
					model.addRow(v);
				}
			} else {
		        ...
		        for (int i = 0; i < objects.size(); i++) {
		            Vector v = new Vector();
		            v.add (new Integer(i)
		            	+ ":" + ((dao.Procedure) objects.get(i)).getMandatoryInconsistency());
		            getRowData((dao.Procedure)objects.get(i), v);
		            model.addRow(v);
		        }
        	}
    ...
    protected void attachRenderers() {
    	...
    	getColumnModel().getColumn(4).setCellEditor(
        		new procedureAllCollectionsEditor( parent_win, 
        			main_handler, objects, this)
        );
    	
    }

    private void getRowData (dao.Procedure object, Vector vector) {
    
        try {
            	object = main_handler.searchProcedureById (object.getId());
   ...
   */
    class procedureAllCollectionsEditor extends ObjectTableCellEditor {
      ...
      public procedureAllCollectionsEditor(
      	  ...
      	  but.addActionListener( new ActionListener() {
      		  public void actionPerformed (ActionEvent event) {
      	    	  ...
      	    	  //but.setEnabled(false);
      	     		dao.Procedure object = (dao.Procedure)objects.get(table.getSelectedRow());
    	    		String id = object.getId();
    	    		//FIXME - object needs to come fresh
    	    		if (main_handler != null){
    	        		try {
    	    				object = main_handler.searchProcedureById (id);
    	    			} catch (Exception e) {
    	    				// TODO Auto-generated catch block
    	    				e.printStackTrace();
    	    			}
    	    	}
    	    	ProcedureViewWindow theProcedureViewWindow =
    	      		new ProcedureViewWindow (parent_win, main_handler, object);

    	        final JFrame frame = new JFrame();
    	        frame.setTitle("Procedure "); /*+ (table.getSelectedRow())*/;
    	        ...
    	        JScrollPane scrollpane = new JScrollPane(theProcedureViewWindow);
    	        frame.add(scrollpane);
    	        
    			// this centers the frame on the screen
    			//frame.setLocationRelativeTo(null);
    	        frame.pack();
       			validateAndShowDetailWindow(frame, (dao.Procedure)object);
