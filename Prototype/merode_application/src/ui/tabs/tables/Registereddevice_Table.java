...
public class Registereddevice_Table extends ObjectTable {
    private MerodeMainEventHandler main_handler;
    private MerodeMainEventHandlerGui parent_win;

    public Registereddevice_Table(MerodeMainEventHandler main_h) {
        this (null, main_h);
    }

    public Registereddevice_Table(MerodeMainEventHandlerGui par_win, 
    ...
    public Registereddevice_Table(MerodeMainEventHandler main_h, String object_id) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            try {
            	obj = main_handler.searchRegistereddeviceById (object_id);
            } catch (Exception ex){
    ...
    public Registereddevice_Table(MerodeMainEventHandler main_h, Collection col) {
        super();
        main_handler = main_h;
        ...
        try {
            ...
            Collections.sort(objects, new Comparator<Object>() {
				...
				public int compare(Object arg0, Object arg1) {
					return (((dao.Registereddevice) arg0).getId()
						.compareTo(((dao.Registereddevice) arg1).getId()));
				}
            });
            createColumns();
            attachRenderers();
	...
	protected void getAllObjects() {
        try {
            Collection c = main_handler.getAllRegistereddevice();
    ...
    protected void createRows() {
    	...
    	try {
            ...
            for (int i = 0; i < all.size(); i++){
            	keys.put(new Integer(i), ((dao.Registereddevice)all.get(i)).getId());
            }
		...
		if ( all != null ){
			if(all.size() > objects.size() && keys != null){
				for (int i = 0; i < objects.size(); i++) {
					...
					for (Entry<Integer, String> entry : keys.entrySet()) {
						if (entry.getValue().equals(
								((dao.Registereddevice) objects.get(i)).getId())) {
							v.add(entry.getKey());
						}
					}
					getRowData((dao.Registereddevice) objects.get(i), v);
					model.addRow(v);
				}
			} else {
		        ...
		        for (int i = 0; i < objects.size(); i++) {
		            Vector v = new Vector();
		            v.add (new Integer(i)
		            	+ ":" + ((dao.Registereddevice) objects.get(i)).getMandatoryInconsistency());
		            getRowData((dao.Registereddevice)objects.get(i), v);
		            model.addRow(v);
		        }
        	}
    ...
    protected void attachRenderers() {
    	...
    	getColumnModel().getColumn(4).setCellEditor(
        		new registereddeviceAllCollectionsEditor( parent_win, 
        			main_handler, objects, this)
        );
    	
    }

    private void getRowData (dao.Registereddevice object, Vector vector) {
    
        try {
            	object = main_handler.searchRegistereddeviceById (object.getId());
   ...
   */
    class registereddeviceAllCollectionsEditor extends ObjectTableCellEditor {
      ...
      public registereddeviceAllCollectionsEditor(
      	  ...
      	  but.addActionListener( new ActionListener() {
      		  public void actionPerformed (ActionEvent event) {
      	    	  ...
      	    	  //but.setEnabled(false);
      	     		dao.Registereddevice object = (dao.Registereddevice)objects.get(table.getSelectedRow());
    	    		String id = object.getId();
    	    		//FIXME - object needs to come fresh
    	    		if (main_handler != null){
    	        		try {
    	    				object = main_handler.searchRegistereddeviceById (id);
    	    			} catch (Exception e) {
    	    				// TODO Auto-generated catch block
    	    				e.printStackTrace();
    	    			}
    	    	}
    	    	RegistereddeviceViewWindow theRegistereddeviceViewWindow =
    	      		new RegistereddeviceViewWindow (parent_win, main_handler, object);

    	        final JFrame frame = new JFrame();
    	        frame.setTitle("Registereddevice "); /*+ (table.getSelectedRow())*/;
    	        ...
    	        JScrollPane scrollpane = new JScrollPane(theRegistereddeviceViewWindow);
    	        frame.add(scrollpane);
    	        
    			// this centers the frame on the screen
    			//frame.setLocationRelativeTo(null);
    	        frame.pack();
       			validateAndShowDetailWindow(frame, (dao.Registereddevice)object);
      ...
      private void validateAndShowDetailWindow(JFrame frame, 
      			final dao.Registereddevice object) {
