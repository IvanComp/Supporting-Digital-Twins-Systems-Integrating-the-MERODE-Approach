 ...
 */
public class ProductStateEnded extends ProductState {

    public static ProductStateEnded getObject (org.hibernate.Session sess) throws org.hibernate.HibernateException {
	    ProductStateEnded state = null;
	    // Search in database
	    java.util.Collection states = findStateByName (sess, "ended");
	    if ( states != null && !states.isEmpty() )
	        state = (ProductStateEnded)states.iterator().next();
	    if ( state == null ) {
	        state = new ProductStateEnded();
	        // Save in database
	        sess.save (state);
	    }
	    return state;
